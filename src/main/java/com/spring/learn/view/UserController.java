...
public class UserController {
	...
	public String login(HttpServletRequest request, UserVO vo, HttpServletResponse response){
	 ...
	 if (user != null) { // 로그인 성공
	  ...
	  try {
	   ...
	   w.write("<script>alert('로그인 실패!');location.href='/learn/Common/index.jsp';</script>");
	   w.flush();
	   w.close();
	  } catch(Exception e) {
	   e.printStackTrace();
	...
	/*
	 * @RequestMapping(value = "/login.do", method = RequestMethod.POST) // 4.3버전 부터
	 * 사용가능 public String login(HttpServletRequest request, UserVO vo){
	 ...
	 * String str = null;
	 * 
	 * //3. 화면 네비게이션(화면전환, 화면이동) // 로그인 성공 : 게시글 목록 보여주기 // 로그인 실패 : 로그인 화면으로 이동 if
	 * (user != null) { System.out.println(">> 로그인 성공!!!"); HttpSession session =
	 * request.getSession(); UserVO vo2 = userService.getUser(vo);
	 * session.setAttribute("user", vo2); if (vo2.getGrade().equals("관리자")) { str =
	 * "/Admin/adminIndex.jsp"; //회원등급이 관리자일 경우 관리자 페이지로 전환 } else { str =
	 * "/Common/index.jsp"; } } else { System.out.println(">> 로그인 실패~~~"); } return
	 * str; }
	 */
