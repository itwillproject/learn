...
<title>글상세페이지</title>
	...
	$(document).ready(function() {
		...
		function QnaBoardLikeToggleCommon(){
			var gogo = '${pageContext.request.contextPath}/board/QnaBoardLikeToggle.do?fboardNo=' + fboardNo;
		...
		function uploadSummernoteImageFile(file, editor) {
		 ...
		 function reportBoard(){
			 ...
			 var gogo = '${pageContext.request.contextPath}/board/insertBoardReport.do?boardType=freeBoard&boardNo=' + fboardNo;
	...
	function addComment(frm){
		...
		var gogo = "${pageContext.request.contextPath}/board/addComment.do?fboardNo=${board.fboardNo}";
	...
	function deleteOk(commentNo){
		if (confirm("삭제하시겠습니까??")){
			...
			var gogo = "${pageContext.request.contextPath}/board/delComment.do?fboardNo=${board.fboardNo}&section=fboard&commentNo=";
			gogo += commentNo ;
			commonAjax(gogo);
	...
	function deleteBoard(obj){
		if (confirm("삭제하시겠습니까??")){
			location.href = "${pageContext.request.contextPath}/board/deleteBoard.do?fboardNo=${board.fboardNo }" 
		}
	};
	 
	function modifyBoard(){
		location.href = "${pageContext.request.contextPath}/board/boardModify.do?fboardNo=${board.fboardNo }";
	};
	...
	function deleteCoco(comment2No){
		...
		if (confirm("삭제하시겠습니까??")){
			...
			var gogo = "${pageContext.request.contextPath}/board/deleteCoco.do?fboardNo=${board.fboardNo }&comment2No="+comment2No; 
			commonAjax(gogo);
		}
	}
